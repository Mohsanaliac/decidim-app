...
/* harmony import */ var _forEachState_es_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./forEachState.es.js */ "./node_modules/@graphiql/react/dist/forEachState.es.js");
...
function getVariablesHint(cur, token, options) {
  ...
  if (kind === "StringValue" || kind === "NumberValue" || kind === "BooleanValue" || kind === "NullValue" || kind === "ListValue" && step === 1 || kind === "ObjectField" && step === 2 || kind === "Variable" && step === 2) {
    const namedInputType = typeInfo.type ? (0,graphql__WEBPACK_IMPORTED_MODULE_1__.getNamedType)(typeInfo.type) : void 0;
    if (namedInputType instanceof graphql__WEBPACK_IMPORTED_MODULE_1__.GraphQLInputObjectType) {
      return hintList(cur, token, [{ text: "{" }]);
    } else if (namedInputType instanceof graphql__WEBPACK_IMPORTED_MODULE_1__.GraphQLEnumType) {
    ...
    } else if (namedInputType === graphql__WEBPACK_IMPORTED_MODULE_2__.GraphQLBoolean) {
      return hintList(cur, token, [
        { text: "true", type: graphql__WEBPACK_IMPORTED_MODULE_2__.GraphQLBoolean, description: "Not false." },
        { text: "false", type: graphql__WEBPACK_IMPORTED_MODULE_2__.GraphQLBoolean, description: "Not true." }
      ]);
    }
  }
}
...
function getTypeInfo(variableToType, tokenState) {
  ...
  (0,_forEachState_es_js__WEBPACK_IMPORTED_MODULE_3__.f)(tokenState, (state) => {
    if (state.kind === "Variable") {
      info.type = variableToType[state.name];
    } else if (state.kind === "ListValue") {
      const nullableType = info.type ? (0,graphql__WEBPACK_IMPORTED_MODULE_1__.getNullableType)(info.type) : void 0;
      info.type = nullableType instanceof graphql__WEBPACK_IMPORTED_MODULE_1__.GraphQLList ? nullableType.ofType : null;
    } else if (state.kind === "ObjectValue") {
      const objectType = info.type ? (0,graphql__WEBPACK_IMPORTED_MODULE_1__.getNamedType)(info.type) : void 0;
      info.fields = objectType instanceof graphql__WEBPACK_IMPORTED_MODULE_1__.GraphQLInputObjectType ? objectType.getFields() : null;
